...
impl Repr {
    ...
    pub(crate) fn make_shallow_clone(&self) -> Self {
        if self.is_heap_buffer() {
            ...
            #[cfg(target_pointer_width = "64")]
            if !heap.try_increment_reference_count() {
                ref_count_overflow()
            }
            ...
            #[cfg(target_pointer_width = "32")]
            {
                ...
                // larger than the threshold may be observed if a large number of threads stay
                // between fetch_add ~ if. Using isize::MAX requires an unusual amount of threads
                // to be stuck in this position in order to overflow the reference counter.
                if prev > isize::MAX as usize {
                    ref_count_overflow_after_increment(self)
                }
            }
            ...
            #[cfg(target_pointer_width = "32")]
            #[cold]
            fn ref_count_overflow_after_increment(repr: &Repr) -> ! {
                // Undo the increment performed above before unwinding.
